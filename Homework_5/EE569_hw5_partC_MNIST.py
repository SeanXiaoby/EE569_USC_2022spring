...
def showConfusion(confusionMatrix):
    fig, ax = plt.subplots()
    im = ax.imshow(confusionMatrix, cmap = "Blues")
    ax.figure.colorbar(im)
    for row in range(len(confusionMatrix)):
        for col in range(len(confusionMatrix[row])):
            ax.text(col, row, np.round_(confusionMatrix[row][col], decimals=2), fontsize=9, verticalalignment="center", horizontalalignment="center")
